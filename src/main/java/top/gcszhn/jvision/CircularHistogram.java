...
public class CircularHistogram extends JChart {
     ...
     * 绘制文本标签
     * @param label 标签内容
     * @param rotateCenter_x 标签定位横坐标
     * @param rotateCenter_y 标签定位纵坐标
    ...
    public void loadData(String inputfile, Boolean hasTitle, Integer cutoff) throws FileNotFoundException, IOException {
        ...
        try (LineNumberReader lnr = BasicTool.BufferRead(inputfile)) {
            String line;
            while ((line = lnr.readLine()) != null) {
                if (hasTitle && lnr.getLineNumber() == 1) {
                    CircularHistogram.this.setLegendTextSet(line.split("\t"));//将标题加载为图例文本
                    continue;
                }
